...
export class DetectiveSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
    ...
    async _prepareContext(options) {
        ...
        context.skillsView = context.skills.map((item) => {
            ...
            const baseBlue = ((item.system.hasBlueBase ?? item.system.hasBlue) === true);
            const blueSpecs = item.system.hasBlueSpecs ?? {};
            ...
            for (const [key, niveau] of specEntries) {
                ...
                label: key,
                niveau,
                hasBlue: ((blueSpecs[key] ?? false) === true)
                });
            }
    ...
    _onRender(context, options) {
     ...
     // --- GESTION DE L'IMAGE DE PROFIL ---
        ...
        // PJ : cacher le bouton "supprimer" uniquement pour les compétences
            if (!game.user.isGM) {
            html.querySelectorAll('[data-action="deleteItem"]').forEach(btn => {
                ...
                // Compétences (doublon skill / competence)
                if (item.type === "competence" || item.type === "skill") {
                btn.remove();
                }
            });
            }

            // 🔒 Verrouillage PJ : caractéristiques + ressources (sauf PV)
            if (!game.user.isGM) {

            // --- Caractéristiques ---
            // Tous les inputs numériques des caractéristiques
            html.querySelectorAll(
                'input[name^="system.caracteristiques."]'
            ).forEach(input => {
                input.disabled = true;
            });

            // --- Ressources : Adrénaline / Ancienneté / XP ---
            html.querySelectorAll(
                'input[name="system.ressources.adrenaline.value"],' +
                'input[name="system.ressources.anciennete"],' +
                'input[name="system.ressources.xp"]'
            ).forEach(input => {
                input.disabled = true;
            });

            // --- PV restent éditables ---
            // (on ne touche PAS à system.ressources.pv)
            }
    ...
    async _handleAction(event) {
        ...
        console.log("ACTION", action);
            ...
            // Dé bleu par ligne (Général / Spé)
            const baseBlue = ((item.system.hasBlueBase ?? item.system.hasBlue) === true);
            const specBlue = ((item.system.hasBlueSpecs?.[specKey] ?? false) === true);
            const hasBlue = (kind === "spec" && specKey && specKey !== "base") ? specBlue : baseBlue;

            await item.rollSkill({ seuil, label, tags, hasBlue });
            return;
        }

        // --- TOGGLE DÉ BLEU PAR LIGNE (Général / Spé) ---
        if (action === "toggleBlueLine") {
            const card = button.closest("[data-item-id]");
            const itemId = card?.dataset?.itemId;
            const kind = card?.dataset?.lineKind;
            ...
            // Optionnel : si tu veux réserver au MJ, décommente :
            // if (!game.user.isGM) return;

            if (kind === "spec" && specKey && specKey !== "base") {
                const map = foundry.utils.deepClone(item.system.hasBlueSpecs ?? {});
                map[specKey] = !(map[specKey] === true);
                await item.update({ "system.hasBlueSpecs": map });
            } else {
                const current = ((item.system.hasBlueBase ?? item.system.hasBlue) === true);
                const next = !current;
                // On met aussi à jour le legacy hasBlue pour compat / cohérence
                await item.update({ "system.hasBlueBase": next, "system.hasBlue": next });
            }
            return;
        }
        ...
        if (["editItem", "deleteItem", "rollBurst", "rollWeapon", "toggleEquip", "onRollItem", "toggleFavorite", "manageSpecialisations"].includes(action)) {
            ...
            // Gestion des spécialisations (MJ) : ne dépend pas du document item déjà résolu
        ...
        const open = () => {
            ...
            const rows = Object.entries(specs).map(([key, value]) => `
                <div class="form-group" data-spec-key="${key}" style="display:flex; gap:6px; align-items:center; width:100%;">
                    ...
                    <input type="number" class="spec-level" value="${value}" min="2" style="width:60px; flex:0 0 auto;" />
            ...
            const content = `
                ...
                <p style="margin-top:8px; opacity:.7; font-size:.9em;">
                    Les changements sont appliqués immédiatement.
                </p>
            `;

            const dlg = new Dialog({
                title: `Spécialisations — ${item.name}`,
                content,
                buttons: {
                    close: { label: "Fermer" }
                },
                render: (html) => {
                    ...
                    html.on("click", ".spec-delete", async (ev) => {
                        ev.preventDefault();

                        // Clique possible sur l'icône <i> : on remonte toujours au bouton puis à la ligne
                        const btn = ev.target && ev.target.closest ? ev.target.closest("button.spec-delete") : null;
                        const row = (btn || ev.currentTarget).closest(".form-group");

                        // Clé depuis data-spec-key ; fallback : valeur du champ texte
                        let key = row && row.dataset ? row.dataset.specKey : null;
                        if (!key) {
                            const nameInput = row ? row.querySelector('input[type="text"]') : null;
                            key = nameInput ? nameInput.value.trim() : null;
                        }
                        if (!key) return;

                        // Suppression robuste via l'opérateur "-=" de Foundry (supprime la clé même si l'objet est un DataModel)
                        await item.update({ [`system.specialisations.-=${key}`]: null });

                        dlg.close();
                        open();
                    });

                    // 🔢 Changement de niveau immédiat (min 2)
                    html.on("change", ".spec-level", async (ev) => {
                        const row = ev.currentTarget.closest(".form-group");
                        const key = row?.dataset?.specKey;
                        if (!key) return;

                        const val = Math.max(2, Number(ev.currentTarget.value));
                        ev.currentTarget.value = val;

                        const next = foundry.utils.deepClone(item.system.specialisations ?? {});
                        next[key] = val;

                        await item.update({ "system.specialisations": next });
                    });

                    // ➕ Ajout immédiat
                    html.find(".spec-add").on("click", async () => {
                        const current = item.system.specialisations ?? {};
                        const chosen = await this._promptChooseSpec(item, mode, options, current);
                        if (!chosen) return;

                        if ((item.system.specialisations ?? {})[chosen] !== undefined) {
                            return ui.notifications.warn("Cette spécialisation existe déjà.");
                        }

                        const next = foundry.utils.deepClone(item.system.specialisations ?? {});
                        next[chosen] = Math.max(2, specAt || 9);
